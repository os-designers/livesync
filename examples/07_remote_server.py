...
async def on_call(ctx: ls.RemoteLayerServicer, x: ls.VideoFrame) -> ls.VideoFrame | None:
    try:
        ...
        if frame.buffer_type == "rgba":
            gray_frame = cv2.cvtColor(frame.data, cv2.COLOR_RGBA2GRAY)  # type: ignore
        elif frame.buffer_type == "bgra":
            gray_frame = cv2.cvtColor(frame.data, cv2.COLOR_BGRA2GRAY)  # type: ignore
        elif frame.buffer_type == "rgb24":
            gray_frame = cv2.cvtColor(frame.data, cv2.COLOR_RGB2GRAY)  # type: ignore
        else:
            raise ValueError(f"Unsupported buffer type: {frame.buffer_type}")

        # Convert back to original format
        if frame.buffer_type == "rgba":
            final_frame = cv2.cvtColor(gray_frame, cv2.COLOR_GRAY2RGBA)  # type: ignore
        elif frame.buffer_type == "bgra":
            final_frame = cv2.cvtColor(gray_frame, cv2.COLOR_GRAY2BGRA)  # type: ignore
        elif frame.buffer_type == "rgb24":
            final_frame = cv2.cvtColor(gray_frame, cv2.COLOR_GRAY2RGB)  # type: ignore
